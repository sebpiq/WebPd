...
(function(){
    ...
    Pd.makeMsgTransfer = function(rawOutArray) {
        ...
        for (i = 0, length = rawOutArray.length;  i < length; i++) {
            ...
            if (matchDollar && matchDollar[0] === rawOutVal) {
                transfer.push(
                    (function(rawOutVal) {
                        var inInd = parseInt(matchDollar[1], 10) - 1;
    ...
    // TODO: patch, $1, $2, ...
    Pd.resolveAbbreviations = function(args, patch) {
        var i, length, arg, matched, cleaned = args.slice(0);
        for (i = 0, length = args.length; i < length; i++) {
            ...
            else if (matched = dollarVarRe.exec(arg) && patch) {
                if (matched[1] === 0) cleaned[i] = patch.id;
            }
        }
    };
    var dollarVarRe = /\$(\d+)/,
